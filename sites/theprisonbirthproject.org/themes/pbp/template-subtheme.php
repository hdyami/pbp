<?php
// $Id: template-subtheme.php,v 1.1.2.6 2008/04/20 14:09:17 johnalbin Exp $
...
function path_to_subtheme() {
  ...
  if (!isset($theme_path)) {
    if ($theme_key == $base_theme) {
      ...
      // their own theme (seperate from pbp). So we need to re-connect those
      // sub-themes with the base pbp theme.
      if ($theme == $theme_key) {
        // Update database
 ...
 */
function _pbp_default($hook, $variables, $suggestions = array(), $extension = '.tpl.php') {
  global $theme_engine;
  ...
  if (!function_exists("_phptemplate_$hook")) {
      ...
      $declaration = <<<EOD
        function _phptemplate_$hook(\$vars, \$suggestions = array()) {
          return _pbp_default('$hook', \$vars, \$suggestions);
        }
EOD;
      eval($declaration);
    }
