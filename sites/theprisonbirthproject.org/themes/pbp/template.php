...
/**
 * @file
 * File which contains theme overrides for the pbp theme.
 *
 * ABOUT
 *
 ...
 *
 * NOTE ABOUT pbp'S TEMPLATE.PHP FILE
 *
 * The base pbp theme is designed to be easily extended by its sub-themes. You
 * shouldn't modify this or any of the CSS or PHP files in the root pbp/ folder.
...
function phptemplate_breadcrumb($breadcrumb) {
  $show_breadcrumb = theme_get_setting('pbp_breadcrumb');
  $show_breadcrumb_home = theme_get_setting('pbp_breadcrumb_home');
  $breadcrumb_separator = theme_get_setting('pbp_breadcrumb_separator');
  $trailing_separator = theme_get_setting('pbp_breadcrumb_trailing') ? $breadcrumb_separator : '';
...
function _phptemplate_variables($hook, $vars = array()) {
  ...
  // Allow the pbp base theme to add or alter variables.
  pbp_preprocess($vars, $hook);
  $function = 'pbp_preprocess_'. $hook;
  if (function_exists($function)) {
    $function($vars, $hook);
  }
  ...
  // with pbp 5.x-0.8 and earlier. New sub-themes should not use this function.
  if (function_exists('pbp_variables')) {
    $vars = pbp_variables($hook, $vars);
  }

  _pbp_hook($hook); // Add support for sub-theme template files.
 ...
 *   The name of the theme function being called (name of the .tpl.php file.)
 */
function pbp_preprocess(&$vars, $hook) {
 ...
 *   A sequential array of variables to pass to the theme template.
 */
function pbp_preprocess_page(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = pbp_id_safe('page-'. $path);
    $body_classes[] = pbp_id_safe('section-'. $section);
    if (arg(0) == 'node') {
      if (arg(1) == 'add') {
        if ($section == 'node') {
 ...
 *   A sequential array of variables to pass to the theme template.
 */
function pbp_preprocess_node(&$vars) {
  global $user;
 ...
 *   A sequential array of variables to pass to the theme template.
 */
function pbp_preprocess_comment(&$vars) {
  global $user;
 ...
 *   A sequential array of variables to pass to the theme template.
 */
function pbp_preprocess_block(&$vars) {
  $block = $vars['block'];
  ...
  $vars['edit_links'] = '';
  if (theme_get_setting('pbp_block_editing') && user_access('administer blocks')) {
 ...
 */
function pbp_add_print_css($url) {
  global $base_path;
